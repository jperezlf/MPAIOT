...
def run(path_to_project):
    ...
    # lasso = Lasso(alpha=0.1, precompute=True, positive=True, selection='random', random_state=42 ,warm_start=True,)
    lasso = Lasso(selection='random', random_state=42, warm_start=True)
    lasso.fit(X_train, Y_train)

    name = path_to_project + '/algorithms/lasso/model/train.dat'

    pickle.dump(lasso, open(name, "wb"))
