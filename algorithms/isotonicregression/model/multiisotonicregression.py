...
class MultiIsotonicRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        """
        X, y = check_X_y(X, y,
                         y_numeric=True)  # In principle, Infs would be OK, but better to complain and let the user handle it
        ...
        def _partition_graph(origV):
            ...
            if len(currpart[0]) - 1 < self.min_partition_size or len(currpart[1]) - 1 < self.min_partition_size:
                ...
                return _partition_graph([origV[idx] for idx in currpart[0][:-1]]) + _partition_graph(
                    [origV[idx] for idx in currpart[1][:-1]])
